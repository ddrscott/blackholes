...
export default class Board extends Phaser.Scene {
    ...
    create() {
        ...
        overlay.setScore(this.score);
        overlay.setStage(this.stage.name);
    ...
    update() {
        ...
        let overlay = this.scene.get('overlay');

        overlay.setStats(`${clicks.total} clicks\n${clicks.per_second} cps\n${this.points.per_second} pps`);
    }
    ...
    addDrop(x) {
        const {stage} = this;
        const radius = this.game.config.width / 15;
    ...
    addPeg(peg, x, y) {
        ...
        if (target) {
            ...
            poly.onHitStart = (other) => {
                ...
                this.score += points;
                this.scene.get('overlay').setScore(this.score);
