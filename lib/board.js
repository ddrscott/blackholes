...
export default class Board extends Phaser.Scene {
    ...
    preload() {
        ...
        for (const mp3 of TRACKS) {
            this.load.audio(`music/${mp3}`, [`music/${mp3}.mp3`]);
        }
    }
    ...
    drawMusicButton() {
        ...
        const music_button = this.add.text(x_increment/2, y_increment * 1.25, '♫  ', {
            ...
            fontSize: 20,
        }).setInteractive().on('pointerdown', (pointer, localX, localY, event) => {
            if (this.sound.mute) {
    ...
    update() {
        let {score_text, stats_text, clicks, score} = this;
...
${clicks.per_second} cps`;
    ...
    addPeg(peg, x, y) {
        ...
        if (target) {
            ...
            poly.onHitStart = (other) => {
                ...
                // this.noteIdx += 1;
                // let detune = parseInt((this.noteIdx % MUSICAL_NOTES) * 100);
                // // let detune = parseInt(target.bonus / 100 * MUSICAL_NOTES * 100);
                // console.log('detune: ', detune);

                // this.sounds.ding.play({detune});
                this.sounds.dings[0].play({
                  detune: parseInt(this.clicks.per_second * 100), // + 1200,
                  volume: 0.5,
                });
                //console.log('detune: ', parseInt(this.clicks.per_second * 100));
            };
        }
    }
    ...
    playTrack(idx) {
        ...
        this.current_track = this.sound.add(`music/${TRACKS[idx]}`);
        this.current_track.play({volume: 0.5, loop: true});
    }
}
