...
export default class Board extends Phaser.Scene {
    ...
    init({stage}) {
        ...
        this.holes = {
            remaining: fetchInt('holes.remaining') || 50,
            max: 100,
            nextAt: fetchInt('holes.nextAt') || 1000,
        };
    }
    ...
    create() {
        ...
        this.events.on('points', ({points, x, y}) => {
            ...
            if (this.score > this.holes.nextAt) {
                ...
                this.holes.nextAt = parseInt(this.holes.nextAt * 1.25);
    ...
    onSaveStats() {
        ...
        localStorage.setItem('holes.remaining', this.holes.remaining);
        localStorage.setItem('holes.nextAt', this.holes.nextAt);
    }
    ...
    generateStars() {
        ...
        if (this.pointsPerSecond.per_second) {
            delay -= this.pointsPerSecond.per_second;
        }
        if (delay < 0) {
           delay = random() * 50;
        }
